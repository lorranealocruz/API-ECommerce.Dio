...
public class ResourceExceptionHandler {
    ...
    public ResponseEntity<StandardError> jsonParseError(HttpMessageNotReadableException e, HttpServletRequest request) {
        ...
        err.setMessage(
                "O corpo da requisição contém valores inválidos ou mal formatados. Verifique os valores de Enums.");
        err.setPath(request.getRequestURI());
        return ResponseEntity.status(status).body(err);
    }
    ...
    public ResponseEntity<StandardError> handleTypeMismatch(
        ...
        err.setStatus(status.value());
        err.setError(error);
        err.setMessage("O formato da data está incorreto, use: yyyy-MM-dd");
        err.setPath(request.getRequestURI());
        return ResponseEntity.status(status).body(err);
    }
    ...
    public ResponseEntity<StandardError> handleGenericException(
        ...
        err.setStatus(status.value());
        err.setError(error);
        err.setMessage("Ocorreu um erro inesperado. Tente novamente mais tarde.");
        err.setPath(request.getRequestURI());
        return ResponseEntity.status(status).body(err);
    }
}
